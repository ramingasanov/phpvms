  ...
  public function handle(PirepUpdated $event)
  {
    if($event->pirep->aircraft) {
      ...
      if($pirep->status === PirepStatus::TAKEOFF || $pirep->status === PirepStatus::LANDED) {
        ...
        if($aircraft && $pirep->status === PirepStatus::LANDED) {
